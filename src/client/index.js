...
//import {create_module_search_client} from './edit/client.js';
//import {create_module_search_client} from './register/client.js';
...
function read_file(){
    if (reader.readyState == 4)
    {
		...
		switch (current_loading_file)
		{
			...
			case 1:
				vehicle_edit_div.innerHTML = reader.responseText;
		       	vehicle_edit = create_edit_vehicle(vehicle_search, post_request);
				break;

			// ./register_vehicle/register.html
			case 2:
				vehicle_register_div.innerHTML = reader.responseText;
		       	vehicle_register = create_register_vehicle(vehicle_search, post_request);
				break;
		...
		else
		{
			...
			vehicle_search.set_mod_register (vehicle_register);
...
function show_module(module_id)
{
	...
	switch(module_id)
	{
		case 1: //edit vehicle module
			press_edit_btn_fetch (vehicle_edit);
			break;
	}
...
function init()
{
	...
	document.getElementById("btn_goto_vehicle_register").addEventListener('click',
		function (){
			selected_module = 2;
			show_module (selected_module);
			press_reg_btn_fetch (vehicle_register);
		}
	);
